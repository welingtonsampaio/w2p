 ...
 * @license		http://creativecommons.org/licenses/by-nd/3.0/  Creative Commons
 */
class W2P_Validate_Alnum extends W2P_Validate_Abstract
{
	...
	public function isValid($value=null)
	{
		...
		{
			/**
			 * @see W2P_Filter_Alnum
			 */
			self::$_filter = new W2P_Filter_Alnum();
		}
		...
		if ($value != self::$_filter->filter($value)) {
			$this->setMessage(self::NOT_ALNUM, $this->getMessage(self::NOT_ALNUM, $value));
			$this->_isError = true;
			return false;
		}
	...
	public function getMessage( $messageVariable, $value )
	{
		...
		switch ( $messageVariable )
		{
			...
			case self::NOT_ALNUM	:
				return sprintf( __("'%s' contains characters which are non alphabetic and no digits", 'W2P'), $value );
